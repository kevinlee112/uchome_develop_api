...
//应用的UCenter配置信息(可以到UCenter后台->应用管理->查看本应用->复制里面对应的配置信息进行替换)
    ...
    define('UC_API', 'http://test.sihemob.com'); // UCenter 的 URL 地址, 在调用头像时依赖此常量
    ...
    $_SC = array();
    $_SC['dbhost'] = '10.173.26.150'; //服务器地址
    $_SC['dbhost_slave']                  = '10.44.31.13';
    $_SC['dbuser'] = 'ququsihemob'; //用户
    $_SC['dbpw'] = 'Ququsihemob123456'; //密码
    $_SC['dbcharset'] = 'utf8'; //字符集
    $_SC['pconnect'] = 0; //是否持续连接
    $_SC['dbname'] = 'uchome'; //数据库
    $_SC['tablepre'] = 'uchome_'; //表名前缀
...
//应用的UCenter配置信息(可以到UCenter后台->应用管理->查看本应用->复制里面对应的配置信息进行替换)
    define('UC_CONNECT', 'mysql'); // 连接 UCenter 的方式: mysql/NULL, 默认为空时为 fscoketopen(), mysql 是直接连接的数据库, 为了效率, 建议采用 mysql
    define('UC_DBHOST', '10.173.26.150'); // UCenter 数据库主机
    define('UC_DBUSER', 'ququsihemob'); // UCenter 数据库用户名
    define('UC_DBPW', 'Ququsihemob123456'); // UCenter 数据库密码
    define('UC_DBNAME', 'ucenter'); // UCenter 数据库名称
    define('UC_DBCHARSET', 'utf8'); // UCenter 数据库字符集
    define('UC_DBTABLEPRE', '`ucenter`.uc_'); // UCenter 数据库表前缀
    define('UC_DBCONNECT', '0'); // UCenter 数据库持久连接 0=关闭, 1=打开
