...
//取得单个数据
$thevalue = $list = array();
$_GET['mlid'] = empty($_GET['mlid'])?0:intval($_GET['mlid']);
if($_GET['mlid']) {
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('messagelog')." WHERE mlid='$_GET[mlid]'");
...
if(submitcheck('thevaluesubmit')) {
	...
	if(empty($thevalue['mlid'])) {
		//添加
		inserttable('messagelog', $setarr);
	} else {
		//更新
		updatetable('messagelog', $setarr, array('mlid'=>$thevalue['mlid']));
	}
	...
	cpmessage('do_success', 'admincp.php?ac=messagelog');
}

if(empty($_GET['op'])) {
	
	//浏览列表
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('messagelog')." ORDER BY dateline desc limit 16");
	...
	//添加
	$thevalue = array('mlid' => 0, 'explower'=>0, 'maxattachsize'=>'10', 'maxfriendnum'=>50, 'postinterval'=>60, 'searchinterval'=>60, 'domainlength'=>0);
