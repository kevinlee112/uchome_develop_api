...
if(submitcheck('opsubmit')) {
	$grademap = array("verify"=>1, "delayverify"=>-1, "close"=>-2, "open"=>1, "recommend"=>2, "unrecommend"=>1, "reward"=>2, "unreward"=>0);
	...
	} elseif($_POST['optype'] == 'reward' || $_POST['optype'] == 'unreward') {
                $grade = $grademap[$_POST['optype']];
                if(!empty($_POST['ids']) && rewardevents($_POST['ids'], $grade)) {
                        cpmessage('do_success', $_POST['mpurl']);
                } else {
...
if(empty($_GET['op'])) {
	...
	//处理搜索
	$intkeys = array('eventid', 'uid', 'public', 'grade', "classid", "reward");
...
function rewardevents($eventids, $reward){
        global $_SGLOBAL;
        ...
        if(!in_array($grade, array(-2,-1,0, 1,2))){
                cpmessage('bad_event_grade');// 错误的活动状态
        }

        $newids = $events = $actions = array();
        $query = $_SGLOBAL['db']->query("UPDATE ".tname('event')." SET reward=$reward WHERE eventid IN (".simplode($eventids).")");
	return $eventids;
}
...
function verifyevents($eventids, $grade){
	...
	if($grade == 2){// 需要同时修改推荐时间
		$updatetime=$_SGLOBAL['timestamp'] + 86400;
		$_SGLOBAL['db']->query("UPDATE ".tname("event")." SET grade='$grade',updatetime=$updatetime, recommendtime='$_SGLOBAL[timestamp]' WHERE eventid IN (" . simplode($newids).")");
	} else {
		$_SGLOBAL['db']->query("UPDATE ".tname("event")." SET grade='$grade',updatetime='$_SGLOBAL[timestamp]' WHERE eventid IN (" . simplode($newids).")");
	}
	//通知
	if($note_inserts){
