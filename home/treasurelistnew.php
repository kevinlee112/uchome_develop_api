<?php
    ...
    if($_GET[op]=='record'){
        ...
        $recordList=array();
        $recordQuery=$_SGLOBAL['db_slave']->query('SELECT a.gid,a.icon,a.title,a.fileurl,a.pkgname,a.subtips,a.thirdtips,b.moneylogusername,b.moneylogid,b.moneyloguid FROM '.tname('creditgood')." AS a LEFT JOIN ".tname('moneylog_union')." AS b ON a.gid=b.moneylogtaskid WHERE a.category=3 AND a.available=1 AND b.moneylogtypecategory=51 AND b.moneylogstatus=0 ORDER BY b.dateline DESC");
        while($recordValue=$_SGLOBAL['db_slave']->fetch_array($recordQuery)){
            $recordValue['number']=$_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query('SELECT count(moneylogid) FROM '.tname('moneylog_union').' WHERE moneylogtypecategory=51 AND moneylogtaskid='.$recordValue['gid'].' AND moneylogid<='.$recordValue['moneylogid']));
            $recordValue['time']=date('Y-m-d',$_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query('SELECT dateline FROM '.tname('moneylog_union').' WHERE moneylogtypecategory=51 AND moneylogtaskid='.$recordValue['gid'].' ORDER BY dateline DESC LIMIT 1')));
            $recordValue['mycounted']=$_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query("SELECT count(a.moneylogid) FROM ".tname('moneylog_union')." AS a LEFT JOIN ".tname('space')." AS b ON a.moneyloguid=b.uid WHERE a.moneylogtypecategory=51 AND a.moneylogtaskid=".$recordValue['gid']." AND b.invitecode='".$code."'"));
            $recordValue['wincounted']=$_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query('SELECT count(moneylogid) FROM '.tname('moneylog_union').' WHERE moneylogtypecategory=51 AND moneylogtaskid='.$recordValue['gid'].' AND moneyloguid='.$recordValue['moneyloguid']));
        ...
        while($treasureValue=$_SGLOBAL['db_slave']->fetch_array($treasureQuery)){
            if($treasureValue['price']/100<=$treasureValue['counted']){
                $moneylogStatus=$_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query('SELECT count(moneylogid) AS moneylogStatus FROM '.tname('moneylog_union').' WHERE moneylogtypecategory=51 AND moneylogtaskid='.$treasureValue['gid'].' AND moneylogstatus=0'), 1);
                if($moneylogStatus){
                    continue;
                }
    ...
    include template('admin/tpl/treasurelistnew');
?>