...
function unzip_album($FILE, $filename, $albumid, $title, $topicid=0){
    global $_SC;
    $new_name = $_SC['attachdir'].'./'.$filename;
    writeLogKang($albumid."unzip_album  new_name:".$new_name." |   tmp_name:".$FILE['tmp_name']);
    $tmp_name = $FILE['tmp_name'];
    if(@copy($tmp_name, $new_name)) {
            @unlink($tmp_name);
	writeLogKang($albumid."unzip_album 11111111111");
    } elseif((function_exists('move_uploaded_file') && @move_uploaded_file($tmp_name, $new_name))) {
	writeLogKang($albumid."unzip_album 222222222222");
    } elseif(@rename($tmp_name, $new_name)) {
	writeLogKang($albumid."unzip_album 3333333333333");
    } else {
	writeLogKang($albumid."unzip_album 444444444444444444");
            return cplang('mobile_picture_temporary_failure');
    }

    $zip = new ZipArchive;
    $res = $zip->open($new_name); 

    writeLogKang($albumid."unzip_album unzip file is : " . $res);

    if ($res === TRUE) {
        //靠靠test靠?
        $zip->extractTo($new_name . "_unzip");
        $zip->close();
	$dir = $new_name . "_unzip";
	if ($dh = opendir($dir))
	{
           ...
           {
		if ($file != "." && $file != "..") {
		    $pic_file = getimagesize($new_name . "_unzip/".$file);
		    writeLogKang($albumid."unzip_album list file name: ".$file." pic_file: " . $pic_file['mime']);
		    pic_save_app($new_name . "_unzip/".$file, $file, $pic_file['bits'], $pic_file['mime'], $albumid, $title);
		}
           }
           closedir($dh);
        }
...
//靠靠
function pic_save_app($file_path, $file_name, $file_size, $file_type, $albumid, $title, $topicid=0) {
	...
	if(!in_array($fileext, $allowpictype)) {
		writeLogKang($albumid.'only_allows_upload_file_types');
	}

	//靠靠
	if(!$filepath = getfilepath($fileext, true)) {
		writeLogKang($albumid.'unable_to_create_upload_directory_server');
	}
	...
	writeLogKang($albumid." aaaaaaaaaaaaaaaaaaaaa");
	//靠靠
	...
	writeLogKang($albumid."bbbbbbbbbbbbb");
	//靠靠靠
...
writeLog("1111111111111111111111111111");
	
	//用户组
	if(!checkperm('allowupload')) {
		ckspacelog();
		writeLog("222222222222");
	...
	if(!ckvideophoto('album', array(), 1)) {
		writeLog("44444444444444444444");
		return cplang('inadequate_capacity_space');
	}
	
	//新用户见习
	if(!cknewuser(1)) {
		writeLog("55555555555555555555");
		return cplang('inadequate_capacity_space');
	}

	$maxattachsize = 0;//checkperm('maxattachsize');//单位MB
	writeLog("maxattachsize:".$maxattachsize);
	if($maxattachsize) {//0为不限制
		if($space['attachsize'] + $FILE['size'] > $maxattachsize + $space['addsize']) {
			writeLog("666666666666666666");
			return cplang('inadequate_capacity_space');
		}
	}
	...
	writeLog("pic_save: new_name->".$new_name." tmp_name->".$tmp_nam);
...
function getinfo($uid) {
        ...
        $query = $_SGLOBAL['db_slave']->query("SELECT s.*, sf.qq, sf.msn, sf.sex, sf.email, sf.mobile, sf.event, sf.tags, sf.resideprovince, sf.residecity FROM ".tname('space')." s, ".tname('spacefield')." sf WHERE s.uid = $uid and s.uid=sf.uid LIMIT 1");
	/*$query = $_SGLOBAL['db']->query("SELECT sf.*, s.* FROM ".tname('space')." s LEFT JOIN ".tname('spacefield')." sf ON sf.uid=s.uid WHERE s.uid=$uid");*/
        $space = $_SGLOBAL['db_slave']->fetch_array($query);
        ...
        /*$queryContact = $_SGLOBAL['db']->query('SELECT qq, msn, sex, email, mobile,event,tags,resideprovince,residecity FROM '.tname('spacefield')." WHERE uid = '$uid' limit 1");
        $valueContact = $_SGLOBAL['db']->fetch_array($queryContact);*/
        
	$eventCat = array();
        $queryEvent = $_SGLOBAL['db_slave']->query("SELECT distinct ec.classid, ec.classname FROM ".tname("event") . " as e inner join ".tname("eventclass") . " as ec on e.classid=ec.classid where e.uid=". $uid );
        while($value=$_SGLOBAL['db_slave']->fetch_array($queryEvent)) {
                $value['pic'] = $_SGLOBAL['eventclass'][$value['classid']]['poster'];
                $eventCat[] = $value;
        }
        $space['games'] = $eventCat;
        $eventList = array();
        $queryEventList = $_SGLOBAL['db_slave']->query("SELECT e.eventid, e.uid, e.username, e.dateline, e.title, e.classid, ec.classname FROM " . tname("event")." as e inner join ".tname("eventclass") . " as ec on e.classid=ec.classid where e.uid =".$uid." ORDER BY e.eventid DESC LIMIT 0, 5");
        while($event = $_SGLOBAL['db_slave']->fetch_array($queryEventList)){
                $eventList[] = $event;
        }
...
function getalbums($uid) {
	...
	$albums = array();
	$query = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('album')." WHERE uid='$uid' ORDER BY albumid DESC");
...
//靠靠
function getalbumsbyuid($id, $uid, $myuid, $isshaishai) {
	global $_SGLOBAL;

	$query = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('album')." WHERE albumid='$id'  ORDER BY albumid DESC limit 1");
	$value = $_SGLOBAL['db_slave']->fetch_array($query);
	$uid=$value["uid"];
	$classid = $value["classid"];
                ...
                if ($value['sid'] > 0) {
                        $sid = $value['sid'];
                        $querySub = $_SGLOBAL['db_slave']->query("SELECT title FROM ".tname('subject')." WHERE sid='$sid'");
                        writeLog("SELECT title FROM ".tname('subject')." WHERE sid='$sid'");
                        while($valueSub = $_SGLOBAL['db_slave']->fetch_array($querySub)) {
	...
	//靠靠
	$query2 = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('pic')." WHERE albumid='$id'  ORDER BY dateline DESC LIMIT 0,10");
	$firstPicId = "";
	while ($value2 = $_SGLOBAL['db_slave']->fetch_array($query2)) {
		$value2['pic'] = pic_get($value2['filepath'], $value2['thumb'], $value2['remote']);
		$value['piclist'][] = $value2;
		if (empty($firstPicId)) {
		    $firstPicId = $value2['picid'];
		    $queryClick = $_SGLOBAL['db_slave']->query("SELECT picid,click_9,click_10,hot FROM ".tname('pic')." WHERE picid='$firstPicId'");
	    if($valueClick = $_SGLOBAL['db_slave']->fetch_array($queryClick)) {
			if ($classid > 0 && $isshaishai==1) {
			    $value['albumclick'] = $valueClick['click_10'];
			}elseif ($isshaishai==2) {
                            $value['albumclick'] = $valueClick['click_9'];
                        } else {
			    $value['albumclick'] = $valueClick['click_9'];
			}
		    }

			$queryIs = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('clickuser')." WHERE uid='$myuid' AND id='$firstPicId' AND idtype='picid'");
        		while($valueIs = $_SGLOBAL['db_slave']->fetch_array($queryIs)) {
        			$value['isalbumclick'] = 1;
        		}

		}

	}
	$value['replyid'] = $firstPicId;
	$replycount = $_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query("SELECT COUNT(*) FROM ".tname('comment')." WHERE id='$firstPicId' AND idtype='picid'"),0);
        $listreply = array();
        if($replycount) {
                $queryreply = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('comment')." WHERE id='$firstPicId' AND idtype='picid' ORDER BY dateline LIMIT 0,6");
                while ($valuereply = $_SGLOBAL['db_slave']->fetch_array($queryreply)) {
                        realname_set($valuereply['authorid'], $valuereply['author']);		
                        $avatar_exists = ckavatar($valuereply['authorid']);
                        if ($avatar_exists ==1) {
                                $avatarfile = avatar_file($valuereply['authorid'], 'middle');
                                $valuereply["avatarfile"] = $avatarfile;
                        }

                        $listreply[] = $valuereply;
                }
        }
	$value['replynum'] = $replycount;
	$value['replylist'] = $listreply;
	$value['userinfo'] = getinfo($uid);
	return $value;
}
//事件发布
...
function notification_app_add($uid, $eid, $type, $note, $returnid=0) {
        global $_SGLOBAL;
	...
	$nickName = $_SGLOBAL['supe_nickname'];
	if (empty($nickName)) {
		$nickName = $_SGLOBAL['supe_username'];
	}
        $setarr = array(
                'uid' => $uid,
                'type' => $type,
                'new' => 1,
                'authorid' => $_SGLOBAL['supe_uid'],
                'author' => $nickName,//$_SGLOBAL['supe_username'],
                'note' => addslashes(sstripslashes($note)),
                'dateline' => $_SGLOBAL['timestamp'],
		'eid' => $eid
        );
...
function notification_app_add_ext($uid, $eid, $type,$ext, $note, $returnid=0) {
        global $_SGLOBAL;
...
function friend_cache($uid) {
	...
	$i = $count = 0;
	$query = $_SGLOBAL['db_slave']->query("SELECT * FROM ".tname('friend')." WHERE uid='$uid' AND status='1' ORDER BY num DESC, dateline DESC");
	while ($value = $_SGLOBAL['db_slave']->fetch_array($query)) {
		if($value['fuid']) {
...
writeLogKang(".....................stat 111111");
		if(getcount('statuser', $setarr)) {
			return false;
		} else {
writeLogKang("......................stat 222222");
	...
	if(getcount('stat', array('daytime'=>$nowdaytime))) {
		...
		//插入统计
writeLogKang(".......................stat 33333333333 ==== ".getcount('stat', array('daytime'=>$nowdaytime)));
...
function check_token($uid,$token){
    if($_POST['appversion']>='2.6.2'){
        global $_SGLOBAL;
        $redis=getRedis();
        $queryToken=$redis->get("userToken".$uid);
        if($queryToken==false){
            $queryToken=$_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT token FROM ".tname('space')." WHERE uid=".$uid));
            if(!empty($queryToken)){
                $rate=rand(1,9);
                $redis->set("userToken".$uid,$queryToken."|".$rate);
            }
        }else{
            $queryTokenArr=explode("|",$queryToken);
            $queryToken=$queryTokenArr[0];
            $rate=$queryTokenArr[1];
        }
        if($token!=$queryToken){
            $result['errcode']=10000;
            $result['errmsg']='您的账号状态已过期，请退出账号重新登录';
            ...
            if($rate==0){
                $rand=rand(1,10000);
                $queryToken=md5($uid.time().$rand);
                $rate=rand(1,9);
                $_SGLOBAL['db']->query("UPDATE ".tname('space')." SET token='".$queryToken."' WHERE uid=".$uid);
            }
            $redis->set("userToken".$uid,$queryToken."|".$rate);
            if($token!=$queryToken){
                return $queryToken;
