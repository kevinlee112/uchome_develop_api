...
if($eventid){// 显示活动内容
	...
	if($view == "member"){
		...
		if ($picid) {
			...
			$badpicids = array();
			$queryE = $_SGLOBAL['db']->query("SELECT e.uid FROM ".tname("event")." e  WHERE e.eventid='$eventid'");
        		$event = $_SGLOBAL['db']->fetch_array($queryE);
			$uid = $event['uid'];//$_GET['uid'] ? $_GET['uid'] : $_SGLOBAL['supe_uid'];
		        $albumname = "photoalbumssystem_".$uid."_".$eventid;
			$query = $_SGLOBAL['db']->query("SELECT pic.* from ".tname("pic")." pic,  ".tname("album")." a  WHERE pic.albumid=a.albumid and a.albumname='$albumname' ORDER BY pic.picid DESC LIMIT $start, $perpage");
			writeLogDebug("SELECT pic.* from ".tname("pic")." pic,  ".tname("album")." a  WHERE pic.albumid=a.albumid and a.albumname='$albumname' ORDER BY pic.picid DESC LIMIT $start, $perpage");
