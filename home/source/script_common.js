...
function checkAuto(){
	var moneylogform=document.getElementsByName("moneylogform");
	...
	var moneylogtypecategory=document.getElementsByName("moneylogtypecategory");
	var moneylogstatus=document.getElementsByName("moneylogstatus");
	if(moneylogstatus[0].value!=1){
		tips[0].innerHTML="请选择记录状态为待处理后再执行操作！";
	}else{
		if(moneylogtypecategory[0].value!="11"){
			tips[0].innerHTML="";
			if(confirm('本操作不可恢复，确认继续？'))
				moneylogform[0].submit();
		}else{
			if(optype[0].value=="pass"){
				if(auto[0].value==2||auto[0].value==1){
					tips[0].innerHTML="";
					if(confirm('本操作不可恢复，确认继续？')){
						moneylogform[0].submit();
					}
				}else{
					tips[0].innerHTML="请选择发奖方式后再执行操作！";
				}
			}else{
				if(confirm('本操作不可恢复，确认继续？')){
					tips[0].innerHTML="";
...
function testtaskfuc(){
	var taskmanage=document.getElementsByName("taskmanage");
	var input=taskmanage[0].getElementsByTagName("input");
	var perpage=document.getElementsByName("perpage");
	for(var i=2;i<perpage[0].value*3+2;i++){//6表示默认从输入反馈次数和结算次数开始进行检测
		if(typeof(input[i])=="undefined"||(input[i].type!="text"&&input[i].type!="checkbox")){
			break;
		}else{
			if(input[i].value==""){
...
function taskblur(obj){
	if(obj.value==""){
		...
		obj.style.border="2px solid #EEE9E9";
	}
}

function checkTaskValue(){
	var perpage=document.getElementsByName("perpage");
	var taskmanage=document.getElementsByName("taskmanage");
	var input=taskmanage[0].getElementsByTagName("input");
	var tip=document.getElementById("tips");
	var updatesubmit=document.getElementsByName("updatesubmit");
	var feedFile=document.getElementsByName("feedFile");
	var countFile=document.getElementsByName("countFile");
	var count=0;
	var feedStatus=0;
	var countStatus=0;
	var fileType,supportFile;
	for(var i=2;i<perpage[0].value*2+2;i++){//6表示默认从输入反馈次数和结算次数开始进行检测
		if(typeof(input[i])=="undefined"||input[i].type!="text"){
			break;
		}else{
			if(isNaN(input[i].value)&&input[i].value!=""||!isNaN(input[i].value)&&input[i].value<0){
				count++;
			}
		}
	}
	if(count==0){
		...
		tip.innerHTML="输入的结算次数或者反馈次数有误，请重新输入！";
	}
	if(feedFile[0].value!=null&&feedFile[0].value!=""){
		if(feedFile[0].value.lastIndexOf(".")!=-1) {
			fileType = (feedFile[0].value.substring(feedFile[0].value.lastIndexOf(".")+1,feedFile[0].value.length)).toLowerCase();
			suppotFile = new Array();
			suppotFile[0] = "txt";
	...
	if(countFile[0].value!=null&&countFile[0].value!=""){
		if(countFile[0].value.lastIndexOf(".")!=-1) {
			fileType = (countFile[0].value.substring(countFile[0].value.lastIndexOf(".")+1,countFile[0].value.length)).toLowerCase();
			suppotFile = new Array();
			suppotFile[0] = "txt";
			for (var i =0;i<suppotFile.length;i++) {
				if (suppotFile[i]==fileType) {
	...
	if(feedStatus!=2&&countStatus!=2&&count==0){
		taskmanage[0].submit();
	}
}

function daytimechange(obj){
	var taskmanagedaytime1=document.getElementsByName("taskmanagedaytime1");
	var taskmanagedaytime2=document.getElementsByName("taskmanagedaytime2");
	var searchcondition=document.getElementsByName("searchcondition");
	taskmanagedaytime1[0].value=obj.innerHTML;
	taskmanagedaytime2[0].value=obj.innerHTML;
	searchcondition[0].submit();
}

function valuechange(obj){
	var taskmanagegid=document.getElementsByName("taskmanagegid");
	var searchcondition=document.getElementsByName("searchcondition");
	for(var i=0;i<taskmanagegid[0].options.length;i++){
		if(obj==taskmanagegid[0].options[i].value){
			taskmanagegid[0].options[i].selected=true;
		}
	}
	searchcondition[0].submit();
}
...
function appendTask(){
	var taskCategory=document.getElementsByName("set[taskcategory]");
	var taskRate=document.getElementsByName("set[taskRate]");
	taskCategory[0].style.display="inline";
	taskRate[0].style.display="inline";
}

function removeTask(){
	var taskCategory=document.getElementsByName("set[taskcategory]");
	var taskRate=document.getElementsByName("set[taskRate]");
	taskCategory[0].value=0;
	taskCategory[0].style.display="none";
	taskRate[0].value=0;
	taskRate[0].style.display="none";
}

function getTaskCategoryValue(obj,obj1){
	var taskCategory=document.getElementsByName("set[taskcategory]");
	var taskRate=document.getElementsByName("set[taskRate]");
	if(obj!=0&&obj!=undefined){
		taskCategory[0].value=obj;
		taskCategory[0].style.display="inline";
		taskRate[0].value=obj1;
		taskRate[0].style.display="inline";
	}
}

function changeByTaskId(obj1,obj2){
	var moneylogtaskid=document.getElementsByName("moneylogtaskid");
	var moneylogstep=document.getElementsByName("moneylogstep");
	var searchcondition=document.getElementsByName("searchcondition");
	moneylogtaskid[0].value=obj1;
	moneylogstep[0].value=obj2;
	searchcondition[0].submit();
}

function changeByUid(obj){
	var moneyloguid=document.getElementsByName("moneyloguid");
    var moneylogstatus=document.getElementsByName("moneylogstatus");
    var moneylogtypecategory=document.getElementsByName("moneylogtypecategory");
	var searchcondition=document.getElementsByName("searchcondition");
	moneyloguid[0].value=obj;
    moneylogstatus[0].value='';
    moneylogtypecategory[0].value='';
	searchcondition[0].submit();
}

function viewUidSpace(obj){
	window.open("space.php?uid="+obj);
}

function updatetaskid(){
	var searchsubmit=document.getElementsByName("searchsubmit");
	var moneylogtaskid=document.getElementsByName("moneylogtaskid");
	var moneylogstep=document.getElementsByName("moneylogstep");
	var searchcondition=document.getElementsByName("searchcondition");
	moneylogtaskid[0].value="";
	moneylogstep[0].value="";
	searchcondition[0].submit();
}

function delRepeat(){
	var taskmanage=document.getElementsByName("taskmanage");
	var action=taskmanage[0].action+"&op=del";
	taskmanage[0].action=action;
	if(confirm('本操作不可恢复，确认删除？')){
		taskmanage[0].submit();
	}
}

function changeAuto(){
	var autoForm=document.getElementsByName("autoForm");
	var isAuto=document.getElementsByName("isAuto");
	autoForm[0].action=autoForm[0].action+"&isAuto="+isAuto[0].value;
	location.href=autoForm[0].action;
}

function appendStyle(obj){
	var classId=document.getElementById(obj);
