...
if(submitcheck('albumsubmit')) {
	...
	if($_POST['albumop'] == 'creatalbum') {
		writeLog("cp.php albumsubmit createalbum aaaaaaa  ");
	...
	writeLog("cp.php uploadsubmit aaaaaaaaaa ");
	//上传图片
	$albumid = $picid = 0;
	...
	writeLog("cp.php upload submit start save pic eventid" . $eventid . " albumid:".$_POST['albumid']." pic_title: ".$_POST['pic_title']." topicid: ".$_POST['topicid']);
	...
	writeLog("viewAlbumid aaaaaaaaaaaaaaaa");	
	//上传完成发送feed
	if($eventid){//跳到活动页面
	
		writeLog("viewAlbumid bbbbbbbbbbbbbbbb");
		$imgs = array();
		$imglinks = array();
	...
	writeLog("viewAlbumid ccccccccccccccccccccccc ");	
		//相册feed
		if(ckprivacy('upload', 1)) {
			include_once(S_ROOT.'./source/function_feed.php');
			writeLog("viewAlbumid ddddddddddddddd : ".$_POST['opalbumid']);
