...
function dbconnect() {
	...
	if(empty($_SGLOBAL['db_slave'])) {
                $_SGLOBAL['db_slave'] = new dbstuff;
                $_SGLOBAL['db_slave']->charset = $_SC['dbcharset'];
                $_SGLOBAL['db_slave']->connect($_SC['dbhost_slave'], $_SC['dbuser'], $_SC['dbpw'], $_SC['dbname'], $_SC['pconnect']);
        }
}
...
function getAppSpace($username, $passpord) {
	$space = array();
...
function getEventClass($key) {
    ...
    $eventCat = "";
    $queryEvent = $_SGLOBAL['db']->query("SELECT * FROM ".tname("eventclass")." WHERE classid=$key");
    if($value=$_SGLOBAL['db']->fetch_array($queryEvent)) {
        $value['pic'] = $_SGLOBAL['eventclass'][$value['classid']]['poster'];
	$value['classname'] = $value['classname'];
        $value['updatemsg'] = "每周一更新";
        $value['currentactivity'] = "上期榜单";
	$dayWeek = 8 - date("w");
        $value['activitystopmsg'] = "第9期".$dayWeek."天后结束";
	$newclassid = getClassid($key);
        $countpknum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(distinct uid) FROM ".tname('album')." WHERE classid=$newclassid"), 0);
        $value['pknum'] = $countpknum;
...
function getspace($key, $indextype='uid', $auto_open=0) {
	...
	if(empty($_SGLOBAL[$var])) {
		$space = array();
		$query = $_SGLOBAL['db_slave']->query("SELECT sf.*, s.* FROM ".tname('space')." s LEFT JOIN ".tname('spacefield')." sf ON sf.uid=s.uid WHERE s.{$indextype}='$key'");
		if(!$space = $_SGLOBAL['db_slave']->fetch_array($query)) {
			$space = array();
		...
		if($space) {			
			...
			if(empty($space['friend'])) {
				if($space['friendnum']>0) {
					$fstr = $fmod = '';
					$query = $_SGLOBAL['db_slave']->query("SELECT fuid FROM ".tname('friend')." WHERE uid='$space[uid]' AND status='1'");
			...
			$space['privacy'] = empty($space['privacy'])?(empty($_SCONFIG['privacy'])?array():$_SCONFIG['privacy']):unserialize($space['privacy']);
		        ...
		        $space['weixin'] = $space['msn'];
        		$space['province'] = $space['resideprovince'];
        		$space['city'] = $space['residecity'];

			/*$avatar_exists = ckavatar($key);
                        if ($avatar_exists ==1) {
                                $avatarfile = avatar_file($key, 'middle');
                                $space["avatarfile"] = $avatarfile;
                        }*/
...
function getuid($name) {
	...
	$uid = 0;
	$query = $_SGLOBAL['db_slave']->query("SELECT uid,username,name,namestatus FROM ".tname('space')." WHERE ".implode(' OR ', $wherearr)." LIMIT 1");
	if($space = $_SGLOBAL['db_slave']->fetch_array($query)) {
		$uid = $space['uid'];
	}
	return $uid;
...
function runlog($file, $log, $halt=0) {
	...
	$yearmonth = sgmdate('YmdH', $_SGLOBAL['timestamp']);
	$logdir = './data/log/';
...
function getcount($tablename, $wherearr=array(), $get='COUNT(*)') {
	...
	writeLogKang("SELECT $get FROM ".tname($tablename)." WHERE $wheresql LIMIT 1");
	return $_SGLOBAL['db_slave']->result($_SGLOBAL['db_slave']->query("SELECT $get FROM `".tname($tablename)."` WHERE $wheresql LIMIT 1"), 0);
}

function getrequestcount($tablename, $wherearr=array(), $get='COUNT(*)') {
        global $_SGLOBAL;
        if(empty($wherearr)) {
                $wheresql = '1';
        } else {
        ...
        return $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT $get FROM ".tname($tablename)." WHERE $wheresql AND UID <> 0 LIMIT 1"), 0);
}

function getnoticecount($tablename, $wherearr=array(), $get='COUNT(*)') {
        global $_SGLOBAL;
        if(empty($wherearr)) {
                $wheresql = '1';
        } else {
	...
	writeLog("SELECT $get FROM ".tname($tablename)." WHERE $wheresql AND UID <> 0 LIMIT 1");
        return $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT $get FROM ".tname($tablename)." WHERE $wheresql AND UID <> 0 LIMIT 1"), 0);
}
...
function getfriendstatus($uid, $fuid) {
	...
	$query = $_SGLOBAL['db_slave']->query("SELECT status FROM ".tname('friend')." WHERE uid='$uid' AND fuid='$fuid' LIMIT 1");
...
function avatar_file($uid, $size) {
	...
	if(empty($_SGLOBAL[$var])) {
        ...
        if(file_exists("/data/web/app.sihemob.com/data/avatar/".$dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).$typeadd."_avatar_$size.jpg"))
        {
            $_SGLOBAL[$var] =  $dir1.'/'.$dir2.'/'.$dir3.'/'.substr($uid, -2).$typeadd."_avatar_$size.jpg";
        }
        else
		{
            $_SGLOBAL[$var] =  "000/00/00/00".$typeadd."_avatar_$size.jpg";
		}
...
function space_key($space, $appid=0) {
	global $_SCONFIG;
	writeLogKang($_SCONFIG['sitekey']." | ".$space['uid']." | ".(empty($appid)?'':'|'.$appid));
	return substr(md5($_SCONFIG['sitekey'].'|'.$space['uid'].(empty($appid)?'':'|'.$appid)), 8, 8);//16);
}

//获得用户唯一串
function space_key_app($uid, $appid=0) {
        global $_SCONFIG;
        return substr(md5($_SCONFIG['sitekey'].'|'.$uid.(empty($appid)?'':'|'.$appid)), 8, 8);//16);
}
...
function getMoneylogTableName( $dateStart = '', $dateEnd = '')
{
	$lastTableTime = strtotime("2017-04-02");
    if (empty($dateStart) || $dateStart >$lastTableTime)
	{
        $dateStart=$lastTableTime;
	}
    if (empty($dateEnd) || $dateEnd >$lastTableTime)
    {
        $dateEnd=$lastTableTime;
    }

    $strDateStart = date('Y-m', $dateStart);
    $strDateEnd = date('Y-m', $dateEnd);

    $strStart = explode('-',$strDateStart);
    $moneylogTableNames[] = 'moneylog_'.date('Y-m', $dateStart);
    while (strtotime($strDateStart) < strtotime($strDateEnd))
	{
        if ($strStart[1]< 12)
        {
            $strStart[1] += 1;
		...
		{
            $strStart[0] = $strStart[0]+ 1;
            $strStart[1] = 1;
		}
		if ($strStart[1] < 10)
		{
            $strStart[1] = '0'.$strStart[1];
		}
        $strDateStart = implode('-',$strStart);
        array_unshift($moneylogTableNames, 'moneylog_'.$strDateStart);
	}
	return $moneylogTableNames;
}
...
//获取指定动作能获得多少积分
function getrewardappplus($action, $update=1, $uid=0, $tid, $needle='', $setcookie = 1) {
	global $_SGLOBAL, $_SCOOKIE;
        writeLog("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% getreward app  action:".$action."---uid:".$uid);
	$credit = 0;
	$reward = array(
		'credit' => 0,
		'experience' => 0
	...
	if($rule['credit'] || $rule['experience']) {
		...
		if($rule['rewardtype']) {
			...
			if(empty($creditlog)) {
				...
				$setarr = array();
				$clid = $creditlog['clid'];
				writeLog("aaaaaaaaaaaaaaaaaaaa".$rule['cycletype']);
				switch($rule['cycletype']) {
					...
					case 4:		//不限周期
						...
						if($rule['cycletype'] == 1) {
							...
							//判断是否为昨天
writeLog("/////////////////today".$today." dateline:".$creditlog['dateline']." rewardnum:".$rule['rewardnum']);
...
writeLog("qqqqqqqqqqqqrewardnum:".$rule['rewardnum']."----cyclenum".$creditlog['cyclenum']."---rewardnum".$rule['rewardnum']."--".$rule['norepeat']);
...
writeLog("ccccccccccc111-");
//$_SGLOBAL['db']->query("UPDATE ".tname('creditrule')." SET rulename="."\"".$_SGLOBAL['supe_username']."\" WHERE rid=49");
			if($setarr) {
					$setarr['uid'] = $uid;
					$setarr['tid'] = $tid;
                                        $setarr['rid'] = $rule['rid'];
					//$_SGLOBAL['db']->query("UPDATE ".tname('creditlog')." SET ".implode(',', $setarr)." WHERE clid='$creditlog[clid]'");
writeLog("ccccccccccccccc");
					$clid = inserttable('creditlog', $setarr, 1);
				}
...
writeLog(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,");
writeLog("update: ".$update." credit:".$reward['credit']);
...
writeLog("UPDATE ".tname('space')." SET ".implode(',', $setarr)." WHERE uid='$uid'");
...
//获取指定动作能获得多少积分
function getrewardapp($action, $update=1, $uid=0, $needle='', $setcookie = 1) {
	global $_SGLOBAL, $_SCOOKIE;
        writeLog("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% getreward app  action:".$action."---uid:".$uid);
	$credit = 0;
	$reward = array(
...
function getreward($action, $update=1, $uid=0, $needle='', $setcookie = 1) {
	writeLog("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% getreward action:".$action."---uid:".$uid);
	global $_SGLOBAL, $_SCOOKIE;
...
function getRedis() {
   ...
   $redis->auth("Ququmob*8Ququmob*8Qu");
   return $redis;
}

function writeLog($sLog, $sMode = 'a+'){
	if (true) return;
	$sPath = "/data/web/app.sihemob.com/log.txt";
        $iRetries = 0;
        $fp = fopen($sPath, $sMode); 
        do{ 
            if ($iRetries > 0){ 
                usleep(rand(1, 1000));
            }                                                                                                                                                                                                     
            $iRetries += 1;
        }while (!flock($fp, LOCK_EX) and $iRetries <= 3); 
...
function writeLogDebug($sLog, $sMode = 'a+'){
	if (true) return;
        $sPath = "/data/web/app.sihemob.com/log_debug.txt";
        $iRetries = 0;
        $fp = fopen($sPath, $sMode);
        do{
            if ($iRetries > 0){
...
function writeLogLoop($sLog, $sMode = 'a+'){
	if (true) return;
        $sPath = "/data/web/app.sihemob.com/log_loop.txt";
        $iRetries = 0;
        $fp = fopen($sPath, $sMode);
        do{
            if ($iRetries > 0){
...
function writeLogBack($sLog, $sMode = 'a+'){
        if (true) return;
        $sPath = "/data/web/app.sihemob.com/log_back.txt";
        $iRetries = 0;
        $sLog = time()." || ".$sLog;
        $fp = fopen($sPath, $sMode);
        do{
...
function writeLogKang($sLog, $sMode = 'a+'){
        if (true) return;
        $sPath = "/data/web/app.sihemob.com/log_kang.txt";
        $iRetries = 0;
	$sLog = time()." || ".$sLog;
        $fp = fopen($sPath, $sMode);
        do{
...
function writeZuobi($sLog, $sMode = 'a+'){
        $sPath = S_ROOT."/zuobi.txt";
        $iRetries = 0;
        $sLog = time()." || ".$sLog;
        $fp = fopen($sPath, $sMode);
        do{
    ...
    function writeLogZheng($sLog, $sMode = 'a+'){
        if (true) return;
        $sPath = LOG_ZHENG;
        $iRetries = 0;
        $fp = fopen($sPath, $sMode);
...
function writeLogError($sLog, $sMode = 'a+'){
	if (true) return;
        $sPath = "/data/web/app.sihemob.com/log_error.txt";
        $iRetries = 0;
        $fp = fopen($sPath, $sMode);
        do{
            if ($iRetries > 0){
    ...
    function writeLogRedis($sLog, $sMode = 'a+'){
        if (false) return;
        $sPath = "/data/web/app.sihemob.com/log_redis2.txt";
        $iRetries = 0;
        $fp = fopen($sPath, $sMode);
        do{
            if ($iRetries > 0){
...
function getSpecialAlbum($uid, $classid, $myid) {
    global $_SGLOBAL;
    $specailValue = "";
    $wherearr = array();
    $sql = "";
	...
	$tmpValue = getalbumsbyuid($album['albumid'], $album['uid'], $myid, 1);
	$tmpValue['hot'] = $album['hot'];
	$albumlist[] = $tmpValue;
    }
    $specailValue['albumlist'] = $albumlist;
    $specailValue['currentgame'] = getEventClass(14);
    $specailValue['rank'] = "0";
    $specailValue['deadline'] = "";//"30天后结束";
    $specailValue['currentstatus'] = "";//"进行中";

    /*$sqlRank = "select a.albumid,a.uid,a.picid,a.hot,(@i:=@i+1) as i from(select  a.albumid,a.uid,p.picid,p.hot from uchome_album a, uchome_pic p where a.albumid=p.albumid and a.albumname != 'photoalbumsystem' AND  a.classid=$classid group by a.albumid order by p.hot desc) a, (select @i:=0) as it";
    $queryRank = $_SGLOBAL['db']->query($sqlRank);
    while($rank = $_SGLOBAL['db']->fetch_array($queryRank)) {
        if ($uid==$rank['uid']) {
	    $specailValue['rank'] = $rank['i'];
	    break;
	}

    }*/
    $specailValue['rank'] = getRank($uid, $classid);


    return $specailValue;

}
function getRank($uid, $classid) {
    global $_SGLOBAL;
    $rank = 0;
    if (true) return $rank;
    $sqlRank = "select a.albumid,a.uid,a.picid,a.hot,(@i:=@i+1) as i from(select  a.albumid,a.uid,p.picid,p.hot from uchome_album a, uchome_pic p where a.albumid=p.albumid and a.albumname != 'photoalbumsystem' AND  a.classid=$classid group by a.uid order by p.hot desc) a, (select @i:=0) as it";
    $sqlRank = "select aa.albumid,aa.hot,aa.uid,(@i:=@i+1) as i from (select  a.albumid, a.uid, a.classid, pc.hot from uchome_album a , (select p1.albumid, p1.hot from uchome_pic p1 where p1.hot = (select max(hot) from uchome_pic where albumid = p1.albumid) order by p1.hot desc ) as pc where a.classid = $classid AND a.picnum>0 and a.albumid = pc.albumid group by a.uid order by pc.hot desc) as aa, (select @i:=0) as it";
    $queryRank = $_SGLOBAL['db']->query($sqlRank);
    while($rank = $_SGLOBAL['db']->fetch_array($queryRank)) {
        if ($uid==$rank['uid']) {
            $rank = $rank['i'];
...
defined('RAD') or define('RAD', PI / 180.0);
defined('TIMEOUT') or define('TIMEOUT', 1);

function getAroundLatLng($dLat, $dLng, $dRaidus){
        $latitude = $dLat;
        $longitude = $dLng;
        ...
        $dPI = PI();
        $mpdLng = $degree * cos($latitude * ($dPI / 180));
        $dpmLng = 1 / $mpdLng;
...
function sendMsgByCategory($category, $toUser, $categorymoneytypeid) {
    global $_SGLOBAL;
	if ($category < 1) return;
        //$queryMsg = $_SGLOBAL['db']->query("SELECT * FROM ".tname('messagerule')." where category=$category and categorymoneytypeid=$categorymoneytypeid order by dateline desc ");
	if ($category != 4) {
	    $queryMsg = $_SGLOBAL['db']->query("SELECT * FROM ".tname('messagerule')." where category=$category order by dateline desc ");
	}  else {
	    $queryMsg = $_SGLOBAL['db']->query("SELECT * FROM ".tname('messagerule')." where category=$category and categorymoneytypeid=$categorymoneytypeid order by dateline desc ");
	}
        while($msg = $_SGLOBAL['db']->fetch_array($queryMsg)) {
            $id = $msg['mlid'];
            writeLogDebug("send msg: id|".$msg['mlid']." type|".$msg['type']." txtContent|".$msg['txtcontent']."---touser:".$toUser);
            if ($msg['type'] == 0 || $msg['type'] == 1) {
                $from = $msg['from'];
		if (strstr($from, "&")) {
                    $froms = explode("&",$from);
                    $from = $froms[rand(0, (count($froms)-1))];
                }
                $to = explode(',',$toUser);
                $txtContent = $msg['txtcontent'];
		if (strstr($txtContent, "&")) {
		    $txtContents = explode("&",$txtContent);
		    $txtContent = $txtContents[rand(0, (count($txtContents)-1))];
		}
                $ext = array();
...
function sendIMMsg($from, $to, $msgvalue, $ext)
{
	...
	if (count($ext) > 0) {
	   $body['ext'] = $ext;
        ...
        $header = array(_im_get_token());
        $res = _im_curl_request($formgettoken,$patoken,$header);

        $arrayResult =  json_decode($res, true);

        writeLogDebug("...........................................................................");
        writeLogDebug("from".$from." to".$to."   ".$arrayResult);
	$arrayTmp = $arrayResult['data'];
        foreach($arrayTmp as $key => $value) {
                writeLogDebug("from".$from." to".$to."   ".$key.":".$value);
        }
        writeLogDebug("from".$from." to".$to."   ".$arrayResult['data']);
        return $arrayResult ;
}

//先获取app管理员token POST /{org_name}/{app_name}/token
function _im_get_token()
{
        ...
        $patoken=json_encode($body);
        $res = _im_curl_request($formgettoken,$patoken);
        $tokenResult = array();

        $tokenResult =  json_decode($res, true);
        return "Authorization: Bearer ". $tokenResult["access_token"];
}
function _im_curl_request($url, $body, $header = array(), $method = "POST")
{
