...
writeLogError("1111111111");
include_once(S_ROOT.'./source/function_cp.php');
writeLogError("do_app.php 111");
...
writeLogError("-------------- do_app.php op is -> ".$op);
...
if(!@include_once(S_ROOT.'./data/data_eventclass.php')) {
	...
	$feedlist = array();
        ...
        $queryPic = $_SGLOBAL['db']->query("SELECT *  FROM ".tname('album')." as a inner join ".tname('pic')." as p on a.albumid=p.albumid WHERE p.uid='$uid' and a.albumname <> 'photoalbumsystem' ORDER BY p.dateline DESC LIMIT 6");
	writeLog("SELECT *  FROM ".tname('album')." as a inner join ".tname('pic')." as p on a.albumid=p.albumid WHERE p.uid='$uid' and a.albumname <> 'photoalbumsystem' ORDER BY dateline DESC LIMIT 6");
...
/*
	...
	$result['updateurl'] = "http://dload.ququyx.com/upload/XinPlus_V2.2.0_1001_20151225.apk";
	if ($_POST['versionName'] < "2.2.0") {
		writeLog("......... update app, versionName  ".$_POST['versionName']."  channel-".$channel);
	    if ($channel == 1001 || $channel == 1002 || $channel == 1003 || $channel == 1004 || $channel == 1006
		...
		|| $channel == 1020 || $channel == 1021) {
            	$result['status'] = 1;
		$result['updateurl'] = "http://dload.ququyx.com/upload/XinPlus_V2.2.0_".$channel."_20151225.apk";
	    }
        }

        if ($_POST['versionName'] < "2.2.0") {
                writeLog("......... update app, versionName  ".$_POST['versionName']."  channel-".$channel);
            if ($channel == 2001 || $channel == 2002 || $channel == 2003 || $channel == 2004 || $channel == 3001
                || $channel == 3002 || $channel == 3003 || $channel == 3004 || $channel == 4001 || 
		$channel == 4002 || $channel == 4003 || $channel == 4004
		|| $channel == 7001 || $channel == 7002 || $channel == 7003
		|| $channel == 8001 || $channel == 8002) {
                $result['status'] = 1;
		$result['updateurl'] = "http://dload.ququyx.com/upload/XinPlus_V2.2.0_".$channel."_20151225.apk";
            }
        }
	...
	writeLog("******************************************* appupdate channel: ".$channel."===".$_POST['versionName'] < "1.0.0");

	$result['updateinfo'] = "更新说明：\n1、福利大升级，现在开始送¥¥ \n2、签到有红包，1元你不领一发？3、做任务领奖金，秒兑Q币话费\n4、现在升级，立享“双蛋”红包";
	writeLog("0000000000000000000000000000000000000000000000 == ".$result['status']);
	writeLog("**************** update url:   ".$result['updateurl']);
	echo json_encode($result);	
} elseif ($op=='imlogin') {
