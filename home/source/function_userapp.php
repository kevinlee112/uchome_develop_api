...
function _my_get_friends($uid) {
	...
	if(!isset($_SGLOBAL[$var])) {
		...
		if($uid == $space['uid']) {
			...
			$query = $_SGLOBAL['db_slave']->query("SELECT fuid FROM ".tname('friend')." WHERE uid='$uid' AND status='1' ORDER BY dateline DESC");
...
function _my_get_name($uid) {
	...
	if(!isset($_SGLOBAL[$var])) {
		...
		if($uid == $space['uid']) {
			...
			$query = $_SGLOBAL['db_slave']->query("SELECT username,name,namestatus FROM ".tname('space')." WHERE uid='$uid'");
