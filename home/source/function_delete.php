...
function hidefeeds($feedids) {
        ...
        $_SGLOBAL['db']->query("UPDATE ".tname('feed')." SET hide = 1 WHERE feedid IN (".simplode($newfeedids).")");

        return $feeds;
}

function showfeeds($feedids) {
        global $_SGLOBAL;
        ...
        $_SGLOBAL['db']->query("UPDATE ".tname('feed')." SET hide = 0 WHERE feedid IN (".simplode($newfeedids).")");
...
function deleteshares($sids) {
	...
	foreach($shares as $key => $value) {
		...
		if($allowmanage && $value['uid'] != $_SGLOBAL['supe_uid']) {
			...
			writeLog("????????????????????????????????????????????????????????222222222222222222 uid: ".$value[uid]);
			getreward2('delshare',0,$value[uid]);
		}
	}
...
function deletealbums($albumids) {
	...
	$managebatch = checkperm('managebatch');
	$delnum = 0;
	writeLog("SELECT * FROM ".tname('album')." WHERE albumid IN (".simplode($albumids).")");
...
function deleteevents($eventids){
	...
	$allowmanage = true;//checkperm('manageevent');
	$managebatch = true;//checkperm('managebatch');
	$delnum = 0;
	...
	writeLogDebug($_SGLOBAL['supe_uid'].".......................delete delete.".simplode($neweventids));
    //数据删除
