...
class usercontrol extends base {
	...
	function onregister() {
		...
		writeLog("onappregister");
                if(($status = $this->_check_username($username)) < 0) {
			writeLog("onappregister status: "+$status);
                        return $status;
                }
                /*if(($status = $this->_check_email($email)) < 0) {
                        return $status;
                }*/
		writeLog("onappregister add_user password: ". $password);
                $uid = $_ENV['user']->add_user($username, $password, $email, 0, $questionid, $answer);
                return $uid;
        }
	...
	function onlogin() {
		...
		writeLog("user.php onlogin: ".$status." db password: ".$user['password']." user pwd: ".md5($passwordmd5.$user['salt']));
